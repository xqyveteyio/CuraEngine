...
#ifndef INFILL_TRIANGLE_WAVE_INFILL_ADVANCED_H
#define INFILL_TRIANGLE_WAVE_INFILL_ADVANCED_H
...
/*!
 * Pre-computed, model-global template for the advanced (skeleton-driven) triangle wave infill
 * pattern.
 *
 ...
 * Instead, one fixed template wave is derived from all layers at once. In contrast to the
 * simple variant, the wave is not restricted to one straight direction: the medial axis
 * (skeleton) of each part is extracted, and the wave teeth are drawn perpendicular to the
 * skeleton, so that curved and branching shapes are filled along their local direction. Regions
 * without a usable skeleton fall back to the straight axis-aligned wave.
 *
...
public:
     ...
     */
    TriangleWaveAdvancedFillProvider(const std::vector<Shape>& layer_outlines, coord_t line_distance, const double fill_angle);
...
class TriangleWaveInfillAdvanced
{
public:
    /*!
     * Generate the triangle wave pattern from a single outline, without cross-layer template.
     *
     * This is the fallback used when no \ref TriangleWaveAdvancedFillProvider is available (e.g. for
...
#endif // INFILL_TRIANGLE_WAVE_INFILL_ADVANCED_H
