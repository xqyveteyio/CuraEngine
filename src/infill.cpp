    ...
    const SliceMeshStorage* mesh)
{
    ...
    case EFillMethod::MEDIAL_ZIGZAG:
        generateMedialZigzagInfill(result_lines, mesh, layer_idx);
        break;
...
void Infill::generateMedialZigzagInfill(OpenLinesSet& result, const SliceMeshStorage* mesh, const int layer_idx)
{
    // Mesh infill: use the pre-computed lines, whose nodes are anchored to the layer below.
    if (mesh != nullptr && mesh->medial_zigzag_generator != nullptr && layer_idx >= 0)
    {
        result.push_back(inner_contour_.intersection(mesh->medial_zigzag_generator->getLinesForLayer(static_cast<size_t>(layer_idx))));
        return;
    }
