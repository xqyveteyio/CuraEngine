...
#include "BoostInterface.hpp" //For the boost voronoi diagram of the medial zigzag pattern.
...
/*!
 * A wall (or a portion of a wall) as a chain of points with precomputed cumulative arc lengths,
 * so that points can be sampled at arbitrary arc-length positions along the wall.
 */
struct WallChain
{
 ...
 */
bool generateRingZigzag(const SingleShape& part, const coord_t period, OpenLinesSet& zigzag_lines)
{
    ...
    // node periods follows from its length and the reference node period.
    const double axis_length = (outer_wall.total_ + inner_wall.total_) / 2.0;
    const size_t num_periods = std::max<size_t>(3, std::llround(axis_length / period));
 ...
 * medial axis and sample node positions uniformly along it, alternating between the two sides. At each
 * sample position the node is placed where the local normal of the axis (the local 'node cutting
 * plane') first hits the wall on that side. Both sides thereby get the same number of node periods; on
 * curved parts the longer (outer) wall automatically gets a larger actual node spacing than the
 * shorter (inner) wall, and small protrusions of the wall don't disturb the node distribution.
 */
bool generateRibbonZigzag(const SingleShape& part, const coord_t period, OpenLinesSet& zigzag_lines)
{
    const std::optional<OpenPolyline> axis = computeMedialAxisPath(part);
    if (! axis.has_value())
    {
        return false;
    }
    ...
    const size_t num_periods = std::max<size_t>(1, std::llround(axis_length / period));
    ...
    for (size_t node_idx = 0; node_idx < 2 * num_periods; node_idx++)
    {
        // Sample positions are uniform along the medial axis; consecutive nodes alternate between the
        // two sides of the axis and are therefore half a period apart on their respective walls.
        const double s = (0.25 + 0.5 * node_idx) / num_periods * axis_length;
        ...
        // The local normal, pointing towards the side this node belongs to.
        const double side = (node_idx % 2 == 0) ? 1.0 : -1.0;
 ...
 * cross the upper resp. lower wall of the part.
 */
...
void Infill::generateMedialZigzagInfill(OpenLinesSet& result)
{
    ...
    const coord_t period = 2 * line_distance_;
    ...
    coord_t plane_shift = getShiftOffsetFromInfillOriginAndRotation(fill_angle_) + shift_;
    plane_shift = ((plane_shift % line_distance_) + line_distance_) % line_distance_;
    ...
    const Simplify simplifier(std::max<coord_t>(max_resolution_, 100), std::max<coord_t>(max_deviation_, 25), 0);

    OpenLinesSet zigzag_lines;
    for (const SingleShape& part : inner_contour_.splitIntoParts())
    {
        SingleShape simplified{ simplifier.polygon(part) };
        if (simplified.empty() || simplified.outerPolygon().size() < 3)
        {
            continue;
        }
        ...
        if (simplified.size() > 1)
        {
            ...
            generated = generateRingZigzag(simplified, period, zigzag_lines);
        }
        else
        {
            // Elongated part: the two walls are the boundary chains on either side of the medial axis.
            generated = generateRibbonZigzag(simplified, period, zigzag_lines);
        }
        if (! generated)
        {
            generatePlaneZigzag(simplified.outerPolygon(), rotation_matrix, plane_shift, line_distance_, zigzag_lines);
        }
    }
