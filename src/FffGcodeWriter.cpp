    ...
    const SliceLayerPart& part) const
{
    ...
    for (unsigned int combine_idx = 1; combine_idx < part.infill_area_per_combine_per_density[0].size(); combine_idx++)
    {
        ...
        const EFillMethod infill_pattern = mesh.settings.get<EFillMethod>("infill_pattern");
        const bool zig_zaggify_infill = mesh.settings.get<bool>("zig_zaggify_infill") || infill_pattern == EFillMethod::ZIG_ZAG || infill_pattern == EFillMethod::TRIANGLE_WAVE;
    ...
    const SliceLayerPart& part) const
{
    ...
    const bool zig_zaggify_infill = mesh.settings.get<bool>("zig_zaggify_infill") || pattern == EFillMethod::ZIG_ZAG || pattern == EFillMethod::TRIANGLE_WAVE;
    const bool connect_polygons = mesh.settings.get<bool>("connect_infill_polygons");
