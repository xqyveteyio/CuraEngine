    ...
    const SliceLayerPart& part) const
{
    ...
    if (mesh.settings.get<coord_t>("wall_line_count") // Disable feature if no walls - it can leave dangling lines at edges
        ...
        && pattern != EFillMethod::MEDIAL_AXIS_ZIGZAG) // Follows the part centerline; the line distance doesn't map to an area density
    {
        ...
        for (unsigned int combine_idx = 0; combine_idx < part.infill_area_per_combine_per_density_[0].size(); ++combine_idx)
        {
            ...
            if (! support_lines.empty())
            {
                ...
                gcode_layer.addLinesByOptimizer(
                    support_lines,
                    configs[combine_idx],
                    (support_pattern == EFillMethod::ZIG_ZAG || support_pattern == EFillMethod::MEDIAL_AXIS_ZIGZAG) ? SpaceFillType::PolyLines : SpaceFillType::Lines,
                    enable_travel_optimization,
                    wipe_dist,
                    flow_ratio,
                    near_start_location,
